...
-- End of Lua Library inline imports
local ____exports = {}
local Point = __TS__Class()
Point.name = "Point"
function Point.prototype.____constructor(self, coordinates)
    self.x = coordinates[1]
    self.y = coordinates[2]
    self.z = coordinates[3]
end
function Point.sub(self, p1, p2)
    return {x = p1.x - p2.x, y = p1.y - p2.y, z = p1.z - p2.z}
end
function Point.distance(self, p1, p2)
    local res = 0
    for ____, num in ipairs(__TS__ObjectValues(Point:sub(p1, p2))) do
        res = res + math.pow(
            math.abs(num),
            2
        )
...
local function locate(____, x, y, z)
    return x, y, z
end
local CENTRE_POINT = __TS__New(
    Point,
    {locate(nil, 32, 32, 32)}
)
local OUTER_RADIUS = 30
local INNER_RADIUS = 27
local CHUNK_SIZE = 10
local MAX_X = 64
local MAX_Y = 64
local MAX_Z = 64
local NUM_CHUNKS_X = math.ceil(MAX_X / CHUNK_SIZE)
local NUM_CHUNKS_Y = math.ceil(MAX_Y / CHUNK_SIZE)
local NUM_CHUNKS_Z = math.ceil(MAX_Z / CHUNK_SIZE)
local Chunk = __TS__Class()
Chunk.name = "Chunk"
function Chunk.prototype.____constructor(self, x, y, z)
    do
        do
            local ____x_0 = x
            self.chunkX = ____x_0
    ...
    self.minX = self.chunkX * Chunk.CHUNK_SIZE
    self.minY = self.chunkY * Chunk.CHUNK_SIZE
    self.minZ = self.chunkZ * Chunk.CHUNK_SIZE
    self.maxX = math.min(self.minX + Chunk.CHUNK_SIZE, MAX_X)
    self.maxY = math.min(self.minY + Chunk.CHUNK_SIZE, MAX_Y)
    self.maxZ = math.min(self.minZ + Chunk.CHUNK_SIZE, MAX_Z)
    self.id = self.chunkX + self.chunkY * NUM_CHUNKS_X + self.chunkZ * NUM_CHUNKS_X * NUM_CHUNKS_Y
end
function Chunk.prototype.print(self)
    print("Chunk {")
    for ____, ____value in ipairs(__TS__ObjectEntries(self)) do
...
do
    local z = 0
    while z <= NUM_CHUNKS_Z do
        local chunk
        do
            local y = 0
            while y <= NUM_CHUNKS_Y do
                do
                    local x = 0
                    while x <= NUM_CHUNKS_X do
                        chunk = __TS__New(Chunk, x, y, z)
                        x = x + 1
                    end
                end
                y = y + 1
